...
layui.use(['form','jquery',"layer"],function() {
    ...
    if(!/http(s*):\/\//.test(location.href)){
        ...
        if(window.sessionStorage.getItem("lockSystem") != "true" && window.sessionStorage.getItem("showNotice") != "true"){
//            showNotice();
        }
    }
...
//        showNotice();
    ...
    function lockPage(){
        layer.open({
            ...
            content : '<div class="admin-header-lock" id="lock-box">'+
                            '<div class="admin-header-lock-img"><img src="'+userImage+'" class="userAvatar"/></div>'+
    ...
    $(".lockSystem").on("click",function(){
    	layer.prompt({
			...
			layer.close(index);
			window.sessionStorage.setItem("unLockPass",value);
			window.sessionStorage.setItem("lockSystem",true);
	        lockPage();
		});
    })
    
    // 判断是否显示锁屏
    if(window.sessionStorage.getItem("lockSystem") == "true"){
        lockPage();
    }
    ...
    $("body").on("click","#unlock",function(){
        if($(this).siblings(".admin-header-lock-input").val() == ''){
            layer.msg("请输入解锁密码！", {icon: 7});
            $(this).siblings(".admin-header-lock-input").focus();
        }else{
            if($(this).siblings(".admin-header-lock-input").val() == window.sessionStorage.getItem("unLockPass")){
                window.sessionStorage.setItem("lockSystem",false);
