...
layui.define(["element","jquery"],function(exports){
            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                ...
                if(data[i].icon != undefined && data[i].icon != ''){
                    if(data[i].icon.indexOf("layui-icon") != -1){
                    	ulHtml += '<i class="layui-icon '+data[i].icon+' " data-icon="layui-icon '+data[i].icon+'"></i>';
                    }else{
                    	ulHtml += '<i class="custom-icon '+data[i].icon+'" data-icon="'+data[i].icon+'"></i>';
                    }
                }
                ...
                for(var j=0;j<data[i].children.length;j++){
                    ...
                    if(data[i].children[j].icon != undefined && data[i].children[j].icon != ''){
                        if(data[i].children[j].icon.indexOf("layui-icon") != -1){
                        	ulHtml += '<i class="layui-icon '+data[i].children[j].icon+'" data-icon="layui-icon '+data[i].children[j].icon+'"></i>';
                        }else{
                        	ulHtml += '<i class="custom-icon '+data[i].children[j].icon+'" data-icon="'+data[i].children[j].icon+'"></i>';
                        }
                    }
	...
	Tab.prototype.render = function() {
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="page/main.html"><i class="layui-icon layui-icon-home" data-icon="layui-icon layui-icon-home"></i><cite>后台首页</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height()-210);
	...
	Tab.prototype.tabAdd = function(_this){
		...
		if(_this.attr("target") == "_blank"){
			...
			var title = '';
			if(_this.find("i.custom-icon,i.layui-icon").attr("data-icon") != undefined){
				if(_this.find("i.custom-icon").attr("data-icon") != undefined){
					title += '<i class="custom-icon '+_this.find("i.custom-icon").attr("data-icon")+'"></i>';
				}else{
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				//当前窗口内容
				var curmenu = {
					"icon" : _this.find("i.custom-icon").attr("data-icon")!=undefined ? _this.find("i.custom-icon").attr("data-icon") : _this.find("i.layui-icon").attr("data-icon"),
