package com.cctc.fast.core.schedule.zk;
...
import com.cctc.fast.core.schedule.core.Version;
...
public class ZKManager{
    ...
    public enum KEYS {
    	 ...
    	 */
    	isStartSchedule("uncode.schedule.isStartSchedule");
    ...
    private void createZookeeper(final CountDownLatch connectionLatch) throws Exception {
        zk = new ZooKeeper(this.properties.getProperty(KEYS.zkConnectString
                ...
                .getProperty(KEYS.zkSessionTimeout.key)),
                new Watcher() {
                    public void process(WatchedEvent event) {
                        sessionEvent(connectionLatch, event);
                    }
                });
        String authString = this.properties.getProperty(KEYS.userName.key)
                + ":"+ this.properties.getProperty(KEYS.password.key);
